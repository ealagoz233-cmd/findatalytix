...
# ---------- Monte Carlo (_run_gbm) ----------

def test_gbm_deterministic():
    """Aynı seed → birebir aynı sonuç (tekrarlanabilirlik sözleşmesi)."""
    a = main._run_gbm("TEST", mu=0.20, sigma=0.25, seed=42)
    b = main._run_gbm("TEST", mu=0.20, sigma=0.25, seed=42)
    assert a == b
    c = main._run_gbm("TEST", mu=0.20, sigma=0.25, seed=43)
...
def test_gbm_metric_sanity():
    ...
    m = main._run_gbm("XYZ.IS", mu=0.15, sigma=0.30, seed=7)
...
def test_seed_from_prompt_stable():
    ...
    s1 = main._seed_from_prompt("THYAO ile AAPL")
    s2 = main._seed_from_prompt("THYAO ile AAPL")
    s3 = main._seed_from_prompt("GARAN ile MSFT")
